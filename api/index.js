...
export async function POST_summarize(req) {
  ...
  const styleNote = { executive:"Write as a concise executive brief.", detailed:"Write a thorough detailed analysis.", academic:"Write in formal academic style.", bullet:"Use bullet point style.", eli5:"Explain simply like to a 5-year-old." }[style] || "";
  const content = await chat([{
    role: "user",
...
export async function POST_chat(req) {
  ...
  const ctx = docsContext(documents);
  const messages = [
    { role: "system", content: `You are ResearchMind, a helpful AI research assistant. Here are the user's documents:\n\n${ctx}\n\nAnswer questions about these documents. Be helpful and cite document names.` },
    ...history.slice(-6),
